...
const CommunityBoards = () => {
	...
	const {
			...
			data: getNewsArticlesProperties, 
			error: getNewsArticlesError,
			refetch: getNewsArticlesRefetch,
		} = useQuery(GET_BOARD_ARTICLES, {
			fetchPolicy: "network-only", // takror ishlatganimiz uchun network-only
			variables: {input: {...searchCommunity, limit: 6, search: {articleCategory: BoardArticleCategory.NEWS }}},
			notifyOnNetworkStatusChange: true,
			onCompleted: (data: T) => { 
				setNewsArticles(data?.getBoardArticles?.list); // getBoardArticles <= apolle/user/query
			},
		});

		const {
			loading: getFreeArticlesLoading,
			data: getFreeArticlesProperties, 
			...
			notifyOnNetworkStatusChange: true,
			onCompleted: (data: T) => { 
				setFreeArticles(data?.getBoardArticles?.list); // getBoardArticles <= apolle/user/query
			},
		});
