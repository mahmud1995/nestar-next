...
const MemberFollowings = (props: MemberFollowingsProps) => {
	...
	useEffect(() => {getMemberFollowingsRefetch({input: followInquiry}).then()
	}, [followInquiry]);
								...
								<Stack className={'details-box'}>
									...
									<Box className={'info-box'} component={'div'}>
										{follower?.meLiked && follower?.meLiked[0]?.myFavorite ? (
											...
											onClick={() => 
												likeMemberHandler(follower?.followingData?._id, 
												getMemberFollowingsRefetch, followInquiry)}
											/>
										) : (
											<FavoriteBorderIcon
											onClick={() => likeMemberHandler(follower?.followingData?._id,
												getMemberFollowingsRefetch, followInquiry
											)} />
										)}
